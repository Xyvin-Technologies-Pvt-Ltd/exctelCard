...
const Activity = () => {
  ...
  const getActivityStats = () => {
    if (!activities || activities.length === 0)
      return { total: 0, scans: 0, downloads: 0, websiteViews: 0 };

    const scans = activities.filter(
      (activity) => activity.type === "scan"
    ).length;
    const downloads = activities.filter(
      (activity) => activity.type === "download"
    ).length;
    const websiteViews = activities.filter(
      (activity) => activity.type === "website view"
    ).length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <Card>
          ...
          >
            ...
            <div className="ml-4 text-left">
              <h3 className="text-sm font-medium text-gray-500">Website Views</h3>
              <p className="text-2xl font-semibold text-gray-800">
                {stats.websiteViews}
              </p>
            </div>
